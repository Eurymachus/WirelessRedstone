	...
	private class RedstoneEtherGuiPanel extends JPanel implements MouseListener {
		...
		private void drawActiveFrequencies(Graphics2D g2) {
			...
			for ( String i : list.keySet() ) {
				 if ( RedstoneEther.getInstance().getFreqState(ModLoader.getMinecraftServerInstance().getWorldServer(0),i) )
		...
		private void drawNodes(Graphics2D g2) {
			...
			for ( RedstoneEtherNode node : rxs ) {
				if ( RedstoneEther.getInstance().getFreqState(ModLoader.getMinecraftServerInstance().getWorldServer(0),node.freq) )
					g2.setColor(Color.GREEN);
				else
					g2.setColor(Color.RED);
