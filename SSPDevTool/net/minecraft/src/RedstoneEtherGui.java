...
public class RedstoneEtherGui extends JFrame {
	...
	private class RedstoneEtherGuiPanel extends JPanel implements MouseListener {
		...
		private void drawActiveFrequencies(Graphics2D g2) {
			Map<String, Integer> list = RedstoneEther.getInstance()
					.getLoadedFrequencies();
			g2.drawString("Active Freqs:", 5, 20);
			int n = 3;
			for (String i : list.keySet()) {
				if (RedstoneEther.getInstance().getFreqState(
						ModLoader.getMinecraftInstance().theWorld, i))
		...
		private void drawNodes(Graphics2D g2) {
			...
			for (RedstoneEtherNode node : rxs) {
				if (RedstoneEther.getInstance().getFreqState(
						ModLoader.getMinecraftInstance().theWorld, node.freq))
					g2.setColor(Color.GREEN);
				else
					g2.setColor(Color.RED);
				g2.drawString("(" + node.i + "," + node.j + "," + node.k
						+ "):[" + node.freq + "]", 100, n * 10);

				Point[] bound = { new Point(100, (n - 1) * 10),
						new Point(299, (n - 1) * 10 + 9) };
			...
			for (RedstoneEtherNode node : txs) {
				...
				Point[] bound = { new Point(300, (n - 1) * 10),
						new Point(500, (n - 1) * 10 + 9) };
				bounds.put(bound, node);
		...
		public void mouseClicked(MouseEvent arg0) {
			...
			for (Point[] bound : bounds.keySet()) {
				if (arg0.getPoint().x > bound[0].x
						&& arg0.getPoint().x < bound[1].x
						&& arg0.getPoint().y > bound[0].y
						&& arg0.getPoint().y < bound[1].y) {
	...
	private class RedstoneEtherNodeGuiPanel extends JPanel implements
			MouseListener {
		private static final long serialVersionUID = 1L;
		...
		public void drawNode(Graphics2D g2) {
			if (selectedNode != null) {
				g2.drawString("Node: (" + selectedNode.i + "," + selectedNode.j
						+ "," + selectedNode.k + ")", 10, 20);
						...
						"Frequency State: "
								+ RedstoneEther
										...
										.getFreqState(
												...
												selectedNode.freq), 20, 140);

				g2.drawString(
						"Block ID: "
								+ ModLoader.getMinecraftInstance().theWorld
										.getBlockId(selectedNode.i,
												selectedNode.j, selectedNode.k),
						20, 160);
		...
		@Override
		public void mouseClicked(MouseEvent arg0) {
			if (arg0.getPoint().x > 20 && arg0.getPoint().x < 70
					&& arg0.getPoint().y > 200 && arg0.getPoint().y < 220) {
				if (selectedNodeType == 1) {
					RedstoneEther.getInstance().remReceiver(
							ModLoader.getMinecraftInstance().theWorld,
							selectedNode.i, selectedNode.j, selectedNode.k,
							selectedNode.freq);
					WirelessRedstone.blockWirelessR.dropBlockAsItem(ModLoader
							.getMinecraftInstance().theWorld, selectedNode.i,
							selectedNode.j, selectedNode.k, 1, ModLoader
									.getMinecraftInstance().theWorld
									.getBlockMetadata(selectedNode.i,
											selectedNode.j, selectedNode.k));
					...
					RedstoneEther.getInstance().remTransmitter(
							...
							selectedNode.i, selectedNode.j, selectedNode.k,
							selectedNode.freq);
					WirelessRedstone.blockWirelessT.dropBlockAsItem(ModLoader
							.getMinecraftInstance().theWorld, selectedNode.i,
							selectedNode.j, selectedNode.k, 1, ModLoader
									.getMinecraftInstance().theWorld
									.getBlockMetadata(selectedNode.i,
				...
				ModLoader.getMinecraftInstance().theWorld.setBlockWithNotify(
						selectedNode.i, selectedNode.j, selectedNode.k, 0);
			}
		}
