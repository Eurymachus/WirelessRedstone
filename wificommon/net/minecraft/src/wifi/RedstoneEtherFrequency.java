 ...
 * Partitions the wireless nodes into a frequency container and handles business logic.
 * 
 * @author ali4z
 */
public class RedstoneEtherFrequency {
	 ...
	 * Removes transmitters from the register if they are not loaded in the world.
	 * 
	...
	public boolean getState(World world) {
		...
		try {
			...
			for ( RedstoneEtherNode tx: txs.values() ) {
				...
				if ( !RedstoneEtherInjector.isBlockLoaded(world, tx.i, tx.j, tx.k) ) {
	...
	public void setTransmitterState(World world, int i, int j, int k, boolean state) {
		try {
			...
			txLock.readLock();
			LoggerRedstoneWireless.getInstance("RedstoneEtherFrequency").write("setTransmitterState(world, "+i+", "+j+", "+k+", "+state+")", LoggerRedstoneWireless.LogLevel.DEBUG);
	...
	public void addReceiver(RedstoneEtherNode rx) {
		try {
			rxLock.writeLock();
			LoggerRedstoneWireless.getInstance("RedstoneEtherFrequency").write("addTransmitter("+rx.toString()+")", LoggerRedstoneWireless.LogLevel.DEBUG);
			rxs.put(rx,rx);
			rxLock.writeUnlock();
		} catch (InterruptedException e) {
			LoggerRedstoneWireless.getInstance("WirelessRedstone: "+this.getClass().toString()).writeStackTrace(e);
	...
	public void remTransmitter(World world, int i, int j, int k) {
		try {
			...
			txLock.writeLock();
			LoggerRedstoneWireless.getInstance("RedstoneEtherFrequency").write("remTransmitter(world, "+i+", "+j+", "+k+")", LoggerRedstoneWireless.LogLevel.DEBUG);
	...
	public void remReceiver(int i, int j, int k) {
		try {
			...
			rxLock.writeLock();
			LoggerRedstoneWireless.getInstance("RedstoneEtherFrequency").write("remReceiver("+i+", "+j+", "+k+")", LoggerRedstoneWireless.LogLevel.DEBUG);
	...
	private void updateReceivers(World world) {
		try {
			...
			for ( RedstoneEtherNode rx: rxs.values() ) {
				// Add to remove list if block is not loaded.
				if ( !RedstoneEtherInjector.isBlockLoaded(world, rx.i, rx.j, rx.k) ) {
	...
	/**
	 * Fetch the coordinate array of the closest ACTIVE transmitter from a given point on the world.
	 * 
	...
	/**
	 * Fetch the coordinate array of the closest transmitter from a given point on the world.
	 * 
	 * @param i world X coordinate
	 * @param j world Y coordinate
	 * @param k world Z coordinate
