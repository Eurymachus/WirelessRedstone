	...
	private class RedstoneEtherGuiPanel extends JPanel implements MouseListener {
		...
		private void drawActiveFrequencies(Graphics2D g2) {
			...
			for ( String i : list.keySet() ) {
				 if ( RedstoneEther.getInstance().getFreqState(ModLoader.getMinecraftServerInstance().getWorldManager(0),i) )
		...
		private void drawNodes(Graphics2D g2) {
			...
			for ( RedstoneEtherNode node : rxs ) {
				if ( RedstoneEther.getInstance().getFreqState(ModLoader.getMinecraftServerInstance().getWorldManager(0),node.freq) )
					g2.setColor(Color.GREEN);
				else
					g2.setColor(Color.RED);
	...
	private class RedstoneEtherNodeGuiPanel extends JPanel implements MouseListener {
		...
		public void drawNode(Graphics2D g2) {
			if ( selectedNode != null ) {
				...
				g2.drawString("Frequency State: "+RedstoneEther.getInstance().getFreqState(ModLoader.getMinecraftServerInstance().getWorldManager(0),selectedNode.freq), 20, 140);
				
				g2.drawString("Block ID: "+ModLoader.getMinecraftServerInstance().getWorldManager(0).getBlockId(selectedNode.i, selectedNode.j, selectedNode.k), 20, 160);
