...
public class ThreadWirelessClocker implements Runnable {
	...
	private void stop() {
		...
		for ( TileEntityRedstoneWirelessClocker entity: entities ) {
			if ( entity != null )
				((BlockRedstoneWirelessClocker)WirelessClocker.blockClock).setState(
						entity.worldObj, 
	...
	public void remTileEntity(int i, int j, int k) {
		...
		for ( TileEntityRedstoneWirelessClocker entity: entities ) {
			if (
				entity.getBlockCoord(0) == i &&	
				entity.getBlockCoord(1) == j &&	
	...
	public void run() {
		...
		while(running) {
			...
			ticker+=minTicker;
			if ( ticker > 200000000 ) ticker = 0;
			
			try {
				Thread.sleep(minTicker);
			} catch (InterruptedException e) {
				LoggerRedstoneWireless.getInstance("WirelessRedstone.Clocker").writeStackTrace(e);
			}
		}
	}
}
