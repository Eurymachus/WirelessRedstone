...
public class WirelessRemote
{
	...
	{
		ModLoader.openGUI(entityplayer, new GuiRedstoneWirelessRemote(entityplayer, world));
	}
	...
	{
    	if (WirelessRemoteProcess.remote != null && !mouseDown)
        {
    		WirelessRemoteProcess.deactivateRemote(entityplayer, world);
        }

        if (mouseClicked() && WirelessRemoteProcess.remote == null && entityplayer.inventory.getCurrentItem() != null && entityplayer.inventory.getCurrentItem().getItem() == WirelessRemote.itemRemote && gui != null && gui instanceof GuiRedstoneWireless && !entityplayer.isSneaking() && WirelessRemote.ticksInGui > 0)
        {
