...
public class BlockRedstoneWirelessT extends BlockRedstoneWireless {
	 ...
	 * @param i Block ID
	 */
	protected BlockRedstoneWirelessT(int i, float hardness, float resistance) {
		super(i);
	 ...
	 * - Notify self of neighbor change. (forcing transmitter to check for neighboring power sources)
	 */
	@Override
	...
	/**
	 * Is triggered after the Block is removed from the world and before the TileEntity is removed.<br>
	 ...
	 * - Associates the TileEntity with the GUI.
	 * - Opens the GUI.
	 */
	@Override
	protected boolean onBlockRedstoneWirelessActivated(World world, int i, int j, int k, EntityPlayer entityplayer) {
		...
		if ( tileentity instanceof TileEntityRedstoneWirelessT ) {
			WirelessRedstone.guiWirelessT.assTileEntity((TileEntityRedstoneWirelessT)tileentity);
	 ...
	 * - Exit if the neighboring Block is not of the same type.
	 * - If Block is getting powered directly or indirectly, and the power is not already on, set the power on.
	 * - If Block is not getting powered directly or indirectly, and the power is already on, set the power off.
	 */
	@Override
